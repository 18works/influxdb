...
// ReadTagKeysRule matches 'ReadRange |> keys() |> keep() |> distinct()'.
...
type ReadTagKeysRule struct{}

func (rule ReadTagKeysRule) Name() string {
	return "ReadTagKeysRule"
}

func (rule ReadTagKeysRule) Pattern() plan.Pattern {
...
func (rule ReadTagKeysRule) Rewrite(pn plan.Node) (plan.Node, bool, error) {
	...
	newNode := plan.CreatePhysicalNode(fromNode.ID(), &ReadTagKeysPhysSpec{
		ReadRangePhysSpec: *fromSpec.Copy().(*ReadRangePhysSpec),
		ValueColumnName:   keysSpec.Column,
	})
	plan.ReplaceNode(fromNode, newNode)
